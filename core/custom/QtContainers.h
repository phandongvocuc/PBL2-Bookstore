...
public:
    ...
    Vector() = default;
    Vector(initializer_list<T> init) : data_(init) {}

    [[nodiscard]] int size() const { return static_cast<int>(data_.size()); }
    [[nodiscard]] bool isEmpty() const { return data_.empty(); }
    void clear() { data_.clear(); }

    T value(const int index, const T &defaultValue = T()) const {
        const auto idx = static_cast<size_t>(index);
        return idx < data_.size() ? data_[idx] : defaultValue;
    }

    const T &at(const int index) const { return data_.at(static_cast<size_t>(index)); }

    void reserve(const int count) { data_.reserve(static_cast<size_t>(count)); }
    void append(const T &value) { data_.push_back(value); }
    void push_back(const T &value) { data_.push_back(value); }
    void pushBack(const T &value) { data_.push_back(value); }

    T &operator[](const int index) { return data_[static_cast<size_t>(index)]; }
    const T &operator[](const int index) const { return data_[static_cast<size_t>(index)]; }
...
public:
    class ConstIterator {
    public:
        explicit ConstIterator(map<K, V>::const_iterator it) : it_(it) {}
